...
from .focus_discriminator import FocusDiscriminatorTiny
from .dataset_util import SynthDefocusDataset

def main(args):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ds_tr = SynthDefocusDataset(args.data, split="train", img_size=args.size, task="cls")
    ds_va = SynthDefocusDataset(args.data, split="val", img_size=args.size, task="cls")
    ...
    net = FocusDiscriminatorTiny().to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        if loss_va < best:
            best = loss_va
            torch.save(net.state_dict(), out/"focus_discriminator.pt")
    print("saved -> runs/focus_discriminator.pt")
